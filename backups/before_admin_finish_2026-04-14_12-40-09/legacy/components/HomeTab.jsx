...
export default function HomeTab({
  ...
  const socials = useMemo(
    () =>
      Array.isArray(HOME_CONTENT?.socials) && HOME_CONTENT.socials.length
        ? HOME_CONTENT.socials
        : [
  ...
  const trustPoints = useMemo(
    () =>
      Array.isArray(HOME_CONTENT?.stats) && HOME_CONTENT.stats.length
        ? HOME_CONTENT.stats.map((item) => ({
            value: item.value,
            label: lang === "en" ? item.labelEn || item.labelRu || "" : item.labelRu || item.labelEn || "",
          }))
        : lang === "en"
        ? [
  ...
  const dockButtons = [
    { id: "studio", label: copy.sectionStudio, sub: lang === "en" ? "Core" : "Основа" },
    { id: "trust", label: copy.sectionTrust, sub: lang === "en" ? "Proof" : "Доказательства" },
    { id: "process", label: copy.sectionProcess, sub: lang === "en" ? "Flow" : "Поток" },
  ];
              ...
              <TypewriterText
                texts={
                  lang === "en"
                    ...
                    : [
                        "Премиальный визуал для креаторов",
                        "Дизайн, который ощущается дорогим",
                        "Аватарки, превью, баннеры и системы",
                      ]
                }
                theme={th}
              />
                    ...
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 12 }}>
                      <div>
                        ...
                        <div style={{ fontSize: 12, color: "rgba(148,163,184,.74)", marginTop: 6 }}>
                          {allReviews.length} {copy.reviewsLabel} • {avgRating}/5
                        </div>
                      </div>
                      <button
                        onClick={() => {
