...
const PAYMENT_META = {
  ...
  refunded: { ru: "Возврат", en: "Refunded", color: "#c084fc" },
};
...
function paymentTypeLabel(payment, lang) {
  if (payment.type === "topup") {
    return lang === "en" ? "Balance top-up" : "Пополнение баланса";
  }
  return lang === "en" ? "Order payment" : "Оплата заказа";
}
...
function paymentMethodLabel(method, lang) {
  ...
  if (method === "balance") {
    return lang === "en" ? "Internal balance" : "Внутренний баланс";
  }
  return lang === "en" ? "Manual details" : "Ручные реквизиты";
}
    ...
    >
      ...
      <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
        ...
        {order.paymentMethod === "cryptobot" && (
          ...
          >
            {lang === "en" ? "Open payment" : "Открыть оплату"}
          </button>
        )}
      ...
      >
        {expanded ? (lang === "en" ? "Hide details" : "Скрыть детали") : (lang === "en" ? "Track order" : "Отслеживать заказ")}
      </button>
          ...
          >
            ...
            <div style={{ display: "flex", gap: 8 }}>
              <input
                ...
                placeholder={lang === "en" ? "Message for the designer" : "Сообщение дизайнеру"}
                style={{
                  flex: 1,
                  height: 42,
                  borderRadius: 14,
...
export default function ClientHub({
  ...
  const summary = useMemo(() => {
    ...
    return { activeOrders, paidPayments };
  ...
  // Блокировка скролла при открытии модалки (без scrollTo)
  useEffect(() => {
    if (!topupOpen) return;
    ...
    // Фиксируем body на месте вместо скрытия overflow
    ...
    >
      <div
        style={{
          ...
          background: `linear-gradient(180deg, ${th.card} 0%, ${th.surface} 100%)`,
          boxShadow: "0 16px 34px rgba(0,0,0,.18), inset 0 1px 0 rgba(255,255,255,.04)",
          padding: 22,
          overflow: "hidden",
          position: "relative",
        }}
      >
        ...
        <div style={{ fontSize: 12.5, lineHeight: 1.6, color: th.sub, marginTop: 8, maxWidth: 360 }}>
          {lang === "en"
            ? "Top up, store payment drafts, track progress and keep the communication inside each order card."
        ...
        <>
          ...
          >
            ...
            <div style={{ display: "flex", gap: 10, marginTop: 16, flexWrap: "wrap" }}>
              ...
              >
                {lang === "en" ? "Request manual details" : "Запросить реквизиты"}
              </button>
            </div>
          </div>
          ...
          {recentPayments.length ? (
            <div style={{ display: "grid", gap: 12 }}>
              ...
              >
                ...
                <div className="type-display" style={{ fontSize: 18, color: th.text, marginTop: 6 }}>
                  {lang === "en" ? "Payment history" : "История платежей"}
                </div>
                <div style={{ fontSize: 12, color: th.sub, lineHeight: 1.6, marginTop: 6 }}>
                  {lang === "en"
                    ? "Every top-up and payment draft stays here with current status and quick actions."
                    : "Здесь сохраняются все пополнения и платежные черновики с актуальным статусом и быстрыми действиями."}
                </div>
              </div>
              ...
              {recentPayments.map((payment) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div style={{ display: "flex", justifyContent: "space-between", gap: 12, alignItems: "flex-start", paddingLeft: 8 }}>
                      <div style={{ minWidth: 0, flex: 1 }}>
                        <div className="type-micro" style={{ color: th.sub, fontSize: 10 }}>
                          {lang === "en" ? "Payment entry" : "Запись платежа"}
                        </div>
                        <div style={{ display: "flex", alignItems: "baseline", gap: 10, flexWrap: "wrap", marginTop: 6 }}>
                          <div style={{ fontSize: 20, fontWeight: 900, color: th.text }}>{moneyUsd(payment.amountUSD)}</div>
                          <div style={{ fontSize: 12, color: th.sub }}>{paymentTypeLabel(payment, lang)}</div>
                        </div>
                        <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginTop: 10 }}>
                          ...
                          >
                            {paymentMethodLabel(payment.method, lang)}
                          </span>
                          <span
                            style={{
                              padding: "6px 10px",
                          ...
                          >
                            {fmtDate(payment.createdAt, lang)}
                          </span>
                          {payment.orderNo && (
                            <span
                              style={{
                            ...
                            >
                              {lang === "en" ? "Order" : "Заказ"} #{payment.orderNo}
                            </span>
                          )}
                        </div>
                      </div>
