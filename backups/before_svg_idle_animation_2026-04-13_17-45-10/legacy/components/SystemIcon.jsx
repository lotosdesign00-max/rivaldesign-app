...
export default function SystemIcon({ name = "star", size = 18, color = "currentColor", animated = false, tone = "soft", style, className }) {
  return (
    <>
      <style>{`
        @keyframes rsIconFloat { 0%,100% { transform: translateY(0px); } 50% { transform: translateY(-1.5px); } }
        @keyframes rsIconPulse { 0%,100% { opacity: .92; } 50% { opacity: 1; } }
        @keyframes rsIconSpinSlow { from { transform: rotate(0deg); } to { transform: rotate(360deg); } }
        .rs-icon { transition: transform .22s ease, opacity .22s ease, filter .22s ease; }
        .rs-icon--animated { animation: rsIconFloat 2.8s ease-in-out infinite, rsIconPulse 2.8s ease-in-out infinite; }
        .rs-icon--spin { animation: rsIconSpinSlow 8s linear infinite; transform-origin: 50% 50%; }
        .rs-icon--orbit { animation: rsIconFloat 2.8s ease-in-out infinite, rsIconSpinSlow 11s linear infinite; transform-origin: 50% 50%; }
      `}</style>
      <span
        className={className}
        style={{
      ...
      >
        <Svg
          size={size}
          className={`rs-icon ${animated ? "rs-icon--animated" : ""} ${(name === "tools" || name === "globe") && animated ? "rs-icon--spin" : ""} ${(name === "spark" || name === "party") && animated ? "rs-icon--orbit" : ""}`}
        >
          {iconFor(name)}
        </Svg>
      </span>
