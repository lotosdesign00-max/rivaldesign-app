        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 10, animation: "moreIn .35s ease both" }}>
          ...
          <div style={{ display: "flex", gap: 6, overflowX: "auto", scrollbarWidth: "none" }}>
            {[0, 5, 4, 3].map((rating) => (
              ...
              >
                {rating === 0 ? (t.allRatings || "���") : <span style={{ display: "inline-flex", gap: 2 }}>{Array.from({ length: rating }).map((_, idx) => <SystemIcon key={idx} name="star" size={11} color="#fbbf24" animated />)}</span>}
              </button>
            ))}
          </div>
