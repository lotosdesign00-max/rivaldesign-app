      ...
      >
        ...
        <div style={{ padding: "0 18px 18px" }}>
            ...
            {!videoMissing ? (
              ...
              <div style={{ aspectRatio: "16 / 9", display: "flex", alignItems: "center", justifyContent: "center", padding: 28, background: "radial-gradient(circle at 50% 20%, rgba(255,255,255,.06), transparent 32%), linear-gradient(180deg, rgba(8,8,10,.92) 0%, rgba(8,8,10,.98) 100%)" }}>
                <div style={{ width: "min(420px, 100%)", textAlign: "center" }}>
                  <div style={{ width: 68, height: 68, borderRadius: 999, margin: "0 auto 16px", background: "rgba(255,255,255,.08)", border: "1px solid rgba(255,255,255,.12)", color: "#fff", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 28 }}>▶</div>
          ...
          <div style={{ display: "flex", alignItems: "center", justifyContent: "flex-end", gap: 10, flexWrap: "wrap", paddingTop: 16 }}>
            ...
            >
              {isDone ? `✓ ${copy.completed}` : canComplete ? `+15 XP · ${copy.finishLesson}` : copy.markAsDone}
            </button>
          </div>
        </div>
      </div>
  ...
  return (
    <button
      ...
      style={{
        ...
        overflow: "hidden",
        borderRadius: 24,
        border: `1px solid ${locked ? "rgba(99,102,241,0.1)" : done ? "rgba(16,185,129,0.3)" : hovered ? "rgba(99,102,241,0.3)" : "rgba(99,102,241,0.15)"}`,
        ...
        <div style={{ position: "relative", zIndex: 1, height: "100%", padding: 16, display: "flex", flexDirection: "column" }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 10 }}>
            ...
            <div style={{ 
              ...
              {locked ? "🔒" : done ? copy.moduleStatusDone : progress > 0 ? copy.moduleStatusReady : copy.moduleStatusNew}
            </div>
          </div>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 14, animation: "cardIn .35s ease both" }}>
        ...
        {quizDone ? (
          <div style={{ textAlign: "center", padding: "50px 20px", animation: "cardIn .4s cubic-bezier(.34,1.56,.64,1) both", position: "relative" }}>
            ...
            <div style={{ fontSize: 80, marginBottom: 16, animation: "achieveFloat 4s ease-in-out infinite", position: "relative", zIndex: 1, filter: "drop-shadow(0 10px 20px rgba(0,0,0,0.3))" }}>🏆</div>
            ...
            <button 
              onClick={() => { setQuizMode(false); setQuizIdx(0); setQuizScore(0); setQuizAnswer(null); setQuizDone(false); SFX?.tap?.(); }} 
              style={{ 
          ...
          <div style={{ background: `linear-gradient(180deg, ${th.card} 0%, rgba(8,9,20,1) 100%)`, borderRadius: 28, border: `1px solid ${th.border}`, padding: 28, animation: "cardIn .35s cubic-bezier(.34,1.56,.64,1) both", boxShadow: "0 12px 40px rgba(3,4,8,0.4)" }}>
              ...
              {quizQ.opts.map((opt, i) => {
                ...
                return (
                  <button 
                    ...
                    onMouseEnter={e => { if (quizAnswer === null) { e.currentTarget.style.background = "rgba(255,255,255,0.06)"; e.currentTarget.style.borderColor = "rgba(255,255,255,0.15)"; e.currentTarget.style.transform = "translateX(4px)"; } }}
                    onMouseLeave={e => { if (quizAnswer === null) { e.currentTarget.style.background = bg; e.currentTarget.style.borderColor = brd; e.currentTarget.style.transform = "translateX(0)"; } }}
                  >
                    <span>{opt}</span>
            ...
            {quizAnswer !== null && (
              <div style={{ 
                ...
                <span style={{ fontSize: 16, marginRight: 6 }}>💡</span> {quizQ.exp}
              </div>
            )}
          </div>
        )}
      ...
      <>
        <div style={{ display: "flex", flexDirection: "column", gap: 14, animation: "cardIn .35s ease both" }}>
          ...
          <div style={{ position: "relative", overflow: "hidden", borderRadius: 28, border: `1px solid rgba(255,255,255,0.1)`, background: "#050608", boxShadow: "0 24px 60px rgba(0,0,0,0.35)" }}>
            <img src={selCourse.img} alt={selCourse.title} style={{ position: "absolute", inset: 0, width: "100%", height: "100%", objectFit: "cover", opacity: 0.25, filter: "contrast(1.15) saturate(1.1)" }} />
            ...
            <div style={{ position: "relative", zIndex: 1, padding: "32px 24px 28px", display: "flex", flexDirection: "column", minHeight: 280, justifyContent: "flex-end" }}>
              <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 16 }}>
                ...
                {selCourse.popular && <span style={{ padding: "5px 12px", borderRadius: 999, background: "linear-gradient(135deg, #6366f1, #8b5cf6)", color: "#fff", fontSize: 10, fontWeight: 900, boxShadow: "0 4px 12px rgba(99,102,241,.4)" }}>★ TOP</span>}
              </div>
              ...
              <div style={{ display: "flex", gap: 10, flexWrap: "wrap", borderTop: "1px solid rgba(255,255,255,.1)", paddingTop: 16 }}>
                <span style={{ display: "flex", alignItems: "center", gap: 6, color: "rgba(255,255,255,.9)", fontSize: 12, fontWeight: 700 }}><span style={{ opacity: .6 }}>⏱</span> {selCourse.duration}</span>
                <span style={{ display: "flex", alignItems: "center", gap: 6, color: "rgba(255,255,255,.9)", fontSize: 12, fontWeight: 700 }}><span style={{ opacity: .6 }}>📚</span> {selCourse.lessons} {t.courseLessons}</span>
                <span style={{ display: "flex", alignItems: "center", gap: 6, color: "rgba(255,255,255,.9)", fontSize: 12, fontWeight: 700 }}><span style={{ color: "#fbbf24" }}>★</span> {selCourse.rating}</span>
                <span style={{ display: "flex", alignItems: "center", gap: 6, color: "rgba(255,255,255,.9)", fontSize: 12, fontWeight: 700 }}><span style={{ opacity: .6 }}>👥</span> {selCourse.students.toLocaleString()}</span>
              </div>
            </div>
          </div>
          ...
          <div style={{ background: `linear-gradient(180deg, ${th.card} 0%, rgba(8,9,20,.95) 100%)`, borderRadius: 26, border: `1px solid ${th.border}`, padding: "20px 24px", boxShadow: "0 12px 32px rgba(3,4,8,0.3)" }}>
            ...
            {courseUnlocked ? (
              ...
              >
                ...
                >
                  ✈ {copy.orderCourse} — {fmt(selCourse.price)}
                </button>
              </div>
            )}
          </div>
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
      ...
      <div onClick={() => {
        ...
        SFX.quiz();
        addXPfn(5);
      }} style={{ background: safeStreak.lastQuizDate === today ? th.surface : th.grad, borderRadius: 22, padding: "18px 20px", cursor: safeStreak.lastQuizDate === today ? "not-allowed" : "pointer", boxShadow: th.shadow, display: "flex", alignItems: "center", gap: 14, opacity: safeStreak.lastQuizDate === today ? 0.6 : 1, transition: "all .3s ease" }}>
        <div style={{ fontSize: 40 }}>🧠</div>
        <div style={{ flex: 1 }}>
          <div style={{ fontSize: 15, fontWeight: 900, color: th.btnTxt }}>{safeStreak.lastQuizDate === today ? `✓ ${copy.quizDoneTitle}` : t.quizTitle}</div>
      ...
      <div style={{ display: "flex", gap: 6, overflowX: "auto", scrollbarWidth: "none", WebkitOverflowScrolling: "touch" }}>
        {cats.map((item) => (
          <button key={item} onClick={() => { setCat(item); SFX.filter(); }} style={{ whiteSpace: "nowrap", padding: "7px 14px", borderRadius: 999, fontSize: 12, fontWeight: 700, cursor: "pointer", background: cat === item ? th.grad : "transparent", color: cat === item ? th.btnTxt : th.sub, border: `1px solid ${cat === item ? "transparent" : th.border}`, flexShrink: 0 }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
        {filtered.map((course, i) => {
          ...
          return (
            <div key={course.id} onClick={() => { setSelCourse(course); SFX.course(); }} style={{ background: th.card, borderRadius: 22, border: `1px solid ${th.border}`, overflow: "hidden", cursor: "pointer", animation: `cardIn .35s ease ${i * 0.05}s both` }}>
              <div style={{ position: "relative" }}>
                ...
                <div style={{ position: "absolute", top: 10, left: 10, display: "flex", gap: 6 }}>
                  ...
                  {!courseUnlocked && <span style={{ padding: "3px 8px", borderRadius: 999, background: "rgba(8,8,10,.72)", color: "#fff", fontSize: 9, fontWeight: 900, border: "1px solid rgba(255,255,255,.12)" }}>🔒</span>}
                </div>
