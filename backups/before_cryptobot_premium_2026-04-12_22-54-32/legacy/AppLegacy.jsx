...
export default function App() {
  ...
  const requestTopUp = useCallback((amountInput) => {
    ...
    showToast(lang === "en" ? "Top-up draft created" : "Черновик пополнения создан", "success");
    SFX.success?.();
    return payment;
  }, [lang, showToast]);

  const createCheckoutOrder = useCallback((payload) => {
  ...
  const markPaymentSubmitted = useCallback((paymentId, orderId = null) => {
    ...
    setPaymentHistory(prev => prev.map(payment => {
      ...
      if (payment.type === "topup") {
        return { ...payment, status: "paid", submittedAt: at, paidAt: at };
      }
      return { ...payment, status: "review", submittedAt: at };
    }));
    if (!touched) return false;
    if (matchedPayment?.type === "topup") {
      setWalletBalance(prev => roundMoney(prev + Number(matchedPayment.amountUSD || 0)));
      showToast(lang === "en" ? "Balance credited locally" : "Баланс зачислен локально", "success");
      SFX.success?.();
      return true;
    }
    if (orderId) {
      setOrders(prev => prev.map(order => order.id === orderId
        ? { ...order, status: "payment_review", paymentStatus: "review", updatedAt: at }
        : order
      ));
      pushTimelineEntry(orderId, {
        id: makeEntityId("timeline"),
    ...
    SFX.success?.();
    return true;
  }, [lang, showToast, pushTimelineEntry, pushOrderMessage]);
      ...
      <div style={{ width: "100%", maxWidth: 480, display: "flex", flexDirection: "column", height: "100dvh", position: "relative", zIndex: 1 }}>
        ...
        <main ref={mainRef} style={{ flex: 1, padding: "14px 16px 100px", overflowY: "auto", WebkitOverflowScrolling: "touch", overscrollBehavior: "contain" }}>
          <div key={tab} style={{ animation: "cardIn .35s ease both" }}>
            ...
            {tab === "profile" && <ProfileTab th={th} t={t} lang={lang} streak={streak} achievements={achievements} showToast={showToast} setTab={setTab} setSelectedAchievement={setSelectedAchievement} walletBalance={walletBalance} paymentHistory={paymentHistory} orders={orders} onRequestTopUp={requestTopUp} onMarkPaymentSubmitted={markPaymentSubmitted} onAddOrderMessage={addOrderMessage} onOpenCryptoBot={openCryptoBot} onOpenTelegram={openOrderTelegram} />}
          </div>
        </main>
