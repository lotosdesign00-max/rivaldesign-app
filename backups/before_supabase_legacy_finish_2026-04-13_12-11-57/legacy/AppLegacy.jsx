...
T.kz = { ...T.ru, appName: "Rival Space", homeHero: "\u04d2\u043b\u0435\u043c\u0434\u0456\u043a \u0434\u0435\u04a3\u0433\u0435\u0439\u0434\u0435\u0433\u0456 \u0432\u0438\u0437\u0443\u0430\u043b\u0434\u0430\u0440", navHome: "\u0411\u0430\u0441\u0442\u044b", navGallery: "\u0413\u0430\u043b\u0435\u0440\u0435\u044f", navAI: "AI", navCourses: "\u041a\u0443\u0440\u0441\u0442\u0430\u0440", navPricing: "\u041f\u0440\u0430\u0439\u0441", navMore: "\u041a\u04e9\u0431\u0456\u0440\u0435\u043a", galleryTitle: "\u041f\u043e\u0440\u0442\u0444\u043e\u043b\u0438\u043e", addCart: "\u0421\u0435\u0431\u0435\u0442\u043a\u0435", orderBtn: "\u0422\u0430\u043f\u0441\u044b\u0440\u0443", coursesTitle: "\u041a\u0443\u0440\u0441\u0442\u0430\u0440", toTelegram: "Telegram" };
...
export default function App() {
  if (typeof window !== "undefined") {
    window.__RIVAL_GLOBALS = {
      THEMES, LANGS, PROMO_CODES, SERVICES, SFX, openTg, tgUser, isTg, ls, COURSES, COURSE_CATS, ACHIEVEMENTS,
      getLevel, getLevelProgress, getStreak, saveStreak, addXP, getQuizForToday, getGreeting,
      COURSES_DATA, QUIZ_DATA, DESIGN_PACK_CONFIG, MOCK_DESIGN_PACK, CAT_ICONS, GALLERY, getLevelXP,
      AI_IDEA_PROMPTS_EN, AI_IDEA_PROMPTS_RU, FAQ_DATA, REVIEWS
    };
  }
  ...
  const attachInvoiceMeta = useCallback((paymentId, orderId, invoice) => {
    const nextMeta = {
      ...
      cryptoInvoiceUrl: invoice.mini_app_invoice_url || invoice.bot_invoice_url || invoice.web_app_invoice_url || "",
      cryptoInvoiceUrls: {
  ...
  const settlePaidPayment = useCallback((payment, invoiceStatus = "paid") => {
    const at = new Date().toISOString();
    ...
    if (payment.type === "topup") {
      setWalletBalance(prev => roundMoney(prev + Number(payment.amountUSD || 0)));
    ...
    if (payment.orderId) {
      setOrders(prev => prev.map(order => order.id === payment.orderId ? { ...order, status: "queued", paymentStatus: "paid", cryptoInvoiceStatus: invoiceStatus, updatedAt: at } : order));
    ...
    SFX.success?.();
    return true;
  }, [lang, pushOrderMessage, pushTimelineEntry, showToast]);
  ...
  const requestTopUp = useCallback(async (amountInput) => {
    ...
    try {
      ...
      SFX.success?.();
      return { ...payment, cryptoInvoiceId: invoice.invoice_id, cryptoInvoiceUrl: invoice.mini_app_invoice_url || invoice.bot_invoice_url || invoice.web_app_invoice_url || "" };
    } catch (error) {
    ...
    return payment;
  }, [lang, requestCryptoInvoice, showToast]);
  ...
  const createCheckoutOrder = useCallback(async (payload) => {
    ...
    if (paymentMethod === "cryptobot") {
      try {
        ...
        baseOrder.cryptoInvoiceUrl = invoice.mini_app_invoice_url || invoice.bot_invoice_url || invoice.web_app_invoice_url || "";
      } catch (error) {
  ...
  }, [lang, requestCryptoInvoice, showToast, walletBalance]);
  ...
  const refreshInvoiceStatus = useCallback(async (paymentId) => {
    ...
    try {
      ...
      if (invoice?.status === "paid" && payment.status !== "paid") {
        settlePaidPayment(payment, invoice.status);
        return true;
      }
  ...
  const markPaymentSubmitted = useCallback((paymentId, orderId = null) => {
    ...
    if (orderId) {
      setOrders(prev => prev.map(order => order.id === orderId ? { ...order, status: "payment_review", paymentStatus: "review", updatedAt: at } : order));
      pushTimelineEntry(orderId, {
        id: makeEntityId("timeline"),
        at,
    ...
    SFX.success?.();
    return true;
  }, [lang, paymentHistory, pushOrderMessage, pushTimelineEntry, refreshInvoiceStatus, showToast]);

  const openCryptoBot = useCallback((context) => {
    const invoiceUrl = context?.cryptoInvoiceUrl || context?.cryptoInvoiceUrls?.mini || context?.cryptoInvoiceUrls?.bot || context?.cryptoInvoiceUrls?.web;
    if (invoiceUrl) {
  ...
  return (
    <div style={{ height: "100dvh", background: "transparent", fontFamily: "var(--font-body)", display: "flex", justifyContent: "center", position: "relative", overscrollBehavior: "none" }}>
      <style>{`
        :root{--font-body:"Inter",system-ui,sans-serif;--font-display:"Gilroy-Bold","Gilroy-Heavy","Inter",system-ui,sans-serif;--font-button:"Gilroy-Medium","Gilroy-Bold","Inter",system-ui,sans-serif;--font-number:"Gilroy-Heavy","Gilroy-Bold","Inter",system-ui,sans-serif;--font-micro:"Inter",system-ui,sans-serif;}
        *,*::before,*::after{box-sizing:border-box;-webkit-tap-highlight-color:transparent;-webkit-font-smoothing:antialiased;}
        ...
        html{scroll-behavior:smooth;overscroll-behavior:none;overflow:hidden;height:100%;background:#030408;}
        body{margin:0;padding:0;overflow:hidden;height:100%;overscroll-behavior-y:none;-webkit-overflow-scrolling:touch;font-family:var(--font-body);background:#030408;color:rgba(224,231,255,.95);}
      ...
      <div style={{ width: "100%", maxWidth: 480, display: "flex", flexDirection: "column", height: "100dvh", position: "relative", zIndex: 1 }}>
        {/* Header */}
        <header style={{ flexShrink: 0, position: "sticky", top: 0, zIndex: 100, padding: "12px 12px 0", background: "transparent" }}>
          <div style={{ position: "relative", display: "flex", alignItems: "center", justifyContent: "space-between", minHeight: 58, padding: "12px 14px", borderRadius: 24, background: `linear-gradient(180deg, ${th.nav} 0%, ${th.surface} 100%)`, border: `1px solid ${th.border}`, boxShadow: th.shadow, backdropFilter: "blur(28px)", overflow: "hidden" }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 8, minWidth: 70, position: "relative", zIndex: 1 }}>
              <button onClick={(e) => { 
                ...
                {[0, 1, 2].map(i => <div key={i} style={{ width: i === 1 ? 15 : 20, height: 1.8, borderRadius: 999, background: th.sub, transition: "width .3s ease" }} />)}
              </button>
            </div>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 8, minWidth: 84, justifyContent: "flex-end", position: "relative", zIndex: 1 }}>
              ...
              }} style={{ width: 38, height: 38, borderRadius: 14, border: `1px solid ${th.border}`, background: th.id === "graphite" ? "rgba(255,255,255,.06)" : th.tag, color: th.text, cursor: "pointer", fontSize: 16, display: "flex", alignItems: "center", justifyContent: "center", position: "relative", boxShadow: `inset 0 1px 0 rgba(255,255,255,.06), 0 0 0 0 ${th.glow}`, transition: "all .2s ease" }}>
                🛒
                {cartCount > 0 && (
              ...
              <button
                ...
                title={t.navProfile}
              >
                <span style={{ fontSize: 15, color: th.text, lineHeight: 1 }}>
        ...
        <main ref={mainRef} style={{ flex: 1, padding: "14px 16px 100px", overflowY: "auto", WebkitOverflowScrolling: "touch", overscrollBehavior: "contain" }}>
          <div key={tab} style={{ animation: "cardIn .35s ease both" }}>
            ...
            {tab === "profile" && <ProfileTab th={th} t={t} lang={lang} streak={streak} achievements={achievements} showToast={showToast} setTab={setTab} setSelectedAchievement={setSelectedAchievement} walletBalance={walletBalance} paymentHistory={paymentHistory} orders={orders} onRequestTopUp={requestTopUp} onMarkPaymentSubmitted={markPaymentSubmitted} onRefreshInvoiceStatus={refreshInvoiceStatus} onAddOrderMessage={addOrderMessage} onOpenCryptoBot={openCryptoBot} onOpenTelegram={openOrderTelegram} />}
          </div>
        </main>

        <BottomNav active={tab} onChange={setTab} th={th} t={t} cartCount={cartCount} sfx={SFX} />
      </div>
