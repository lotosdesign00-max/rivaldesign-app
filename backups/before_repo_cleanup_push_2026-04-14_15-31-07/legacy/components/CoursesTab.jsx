    ...
    return (
      <div style={{ display: "flex", flexDirection: "column", gap: 14, animation: "cardIn .35s ease both" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <button onClick={() => { setQuizMode(false); setQuizIdx(0); setQuizScore(0); setQuizAnswer(null); setQuizDone(false); SFX.close(); }} style={{ width: 38, height: 38, borderRadius: 12, border: `1px solid ${th.border}`, background: th.card, color: th.sub, cursor: "pointer", fontSize: 16, display: "flex", alignItems: "center", justifyContent: "center" }}>в†ђ</button>
          <div style={{ flex: 1 }}>
          ...
          <div style={{ background: `linear-gradient(180deg, ${th.card} 0%, rgba(8,9,20,.95) 100%)`, borderRadius: 26, border: `1px solid ${th.border}`, padding: "20px 24px", boxShadow: "0 12px 32px rgba(3,4,8,0.3)" }}>
            ...
            {courseUnlocked ? (
              ...
              >
                ...
                >
                  <SystemIcon name="telegram" size={13} color={th.btnTxt} animated style={{ marginRight: 6 }} /> {copy.orderCourse} — {fmt(selCourse.price)}
                </button>
              </div>
            )}
          </div>
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
      ...
      <div onClick={() => {
        ...
        <span style={{ fontSize: 22, color: th.btnTxt }}>в†’</span>
      </div>
