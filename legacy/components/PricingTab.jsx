...
function ServiceCard({ svc, qty, onAdd, onSub, onExamples, lang, th, isFullPack = false }) {
  ...
  const ui = th ? getThemeTokens(th) : {
    bodyText: "rgba(224,231,255,0.95)",
    mutedText: "rgba(100,116,139,0.80)",
  ...
  return (
    ...
    >
      ...
      <div className="pricing-service-actions" style={{ display: "flex", alignItems: "center", gap: 10 }}>
        {!isFullPack && (
          <button onClick={onExamples} className="pressable" style={{ minHeight: 44, padding: "10px 14px", borderRadius: 12, background: effectiveHovered ? `${ui.accent}16` : `${ui.accent}10`, border: `1px solid ${effectiveHovered ? `${ui.accent}55` : `${ui.accent}22`}`, color: ui.text, fontSize: 10.5, fontWeight: 800, cursor: "pointer", flex: 1 }}>
        ...
        {isFullPack ? (
          ...
          <div className="pricing-service-stepper" style={{ display: "flex", alignItems: "center", gap: 6 }}>
            <button onClick={onSub} disabled={qty === 0} style={{ width: 44, height: 44, borderRadius: 10, border: qty === 0 ? `1px solid ${ui.accent}18` : `1px solid ${ui.accent}55`, background: qty === 0 ? `${ui.accent}0f` : `${ui.accent}18`, color: qty === 0 ? ui.sub : ui.text, fontSize: 18, fontWeight: 900, cursor: qty === 0 ? "not-allowed" : "pointer" }}>?</button>
            ...
            <button onClick={onAdd} style={{ width: 44, height: 44, borderRadius: 10, background: `linear-gradient(135deg, ${ui.accent}, ${ui.accentB})`, color: th?.btnTxt || "#fff", border: "1px solid rgba(255,255,255,0.15)", fontSize: 18, fontWeight: 900, cursor: "pointer" }}>+</button>
          </div>
        )}
      </div>
    </div>
