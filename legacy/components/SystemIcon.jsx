...
function iconFor(name) {
  ...
  switch (normalized) {
    ...
    case "share":
      return <><path d="M8 12a3 3 0 1 0 0-6 3 3 0 0 0 0 6Z" /><path d="M16 18a3 3 0 1 0 0-6 3 3 0 0 0 0 6Z" /><path d="M19 15a3 3 0 1 0 0-6 3 3 0 0 0 0 6Z" /><path d="m8.5 10.5 7 3" /><path d="m16.5 13.5 3 2" /></>;
    default:
...
function SystemIcon({ name = "star", size = 18, color = "currentColor", animated = true, tone = "soft", style, className }) {
  ...
  const motionProfile = (() => {
    if (!animated) return "";
    if (["tools", "globe"].includes(normalizedName)) return "spin";
    if (["spark", "party", "star", "top", "trophy"].includes(normalizedName)) return "twinkle";
    if (["heart", "heart-filled"].includes(normalizedName)) return "beat";
    if (["telegram", "arrow-right", "play", "arrow-left"].includes(normalizedName)) return "glide";
    if (["brain", "idea", "fire", "warning"].includes(normalizedName)) return "pulse";
    return "float";
  })();
  const sceneProfile = (() => {
    if (!animated) return "still";
    if (["wallet"].includes(normalizedName)) return "open";
    if (["lock"].includes(normalizedName)) return "unlock";
    if (["gift"].includes(normalizedName)) return "unwrap";
    if (["telegram", "play", "arrow-right", "arrow-left"].includes(normalizedName)) return "launch";
    if (["search", "eye", "clock", "about", "faq"].includes(normalizedName)) return "focus";
    if (["brain", "idea", "users"].includes(normalizedName)) return "neural";
    if (["tools", "template", "banner", "visual", "custom", "palette", "brush", "texture", "graphics", "mockup", "preview"].includes(normalizedName)) return "build";
    if (["spark", "star", "top", "trophy", "xp", "check", "party", "medal"].includes(normalizedName)) return "flare";
    return "trace";
  })();
  ...
  return (
    <>
      ...
      >
        ...
        <Svg
          size={size}
          className={`rs-icon-svg ${(["tools", "globe"].includes(normalizedName) && animated) ? "rs-icon-svg--spin" : ""}`}
          data-icon={normalizedName}
          data-scene={sceneProfile}
          data-animated={animated ? "true" : "false"}
        >
          {iconFor(normalizedName)}
        </Svg>
        </span>
      </span>
    </>
